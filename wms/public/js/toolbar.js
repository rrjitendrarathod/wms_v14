...
import '../../../../frappe/frappe/public/js/frappe/form/form_viewers';
...
frappe.ui.form.Toolbar = class Toolbar {
	...
	setup_editable_title () {
		...
		this.page.$title_area.find(".title-text").on("click", () => {
			...
			// check if title is updatable --> Old 
			...
			if (me.is_title_editable()) {
				let title_field_label = me.frm.get_docfield(title_field).label;
				let isMedicalCoderFlowDoctype = me.frm.doctype === "Medical Coder Flow";
	
				fields.push({
					label: ("New {0}", [(title_field_label)]),
					fieldname: "title",
					fieldtype: "Data",
					reqd: 1,
					default: me.frm.doc[title_field],
					read_only: isMedicalCoderFlowDoctype ? 1 : 0
				});
			}
			...
			if (me.can_rename()) {
				...
				frappe.user_roles.includes("Super Admin")) && me.frm.doctype === "User"){
                    fields.push(...[{
                        label: ("New Name"),
                        fieldname: "name",
                        fieldtype: "Data",
                ...
                else if (!["Administrator"].includes(frappe.boot.wms.role_profile) && me.frm.doctype == "Employee"){
                    fields.push(...[{
                        label: ("New Name"),
                        fieldname: "name",
                        fieldtype: "Data",
...
// Hiding the Currently Viewing
frappe.ui.form.FormViewers = class CustomFormViewers extends frappe.ui.form.FormViewers {
    refresh() {
