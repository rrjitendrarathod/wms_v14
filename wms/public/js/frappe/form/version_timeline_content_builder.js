	...
	['added', 'removed'].forEach(function(key) {
		if (data[key] && data[key].length) {
			...
			let parts = (data[key] || []).map(function(p) {
				...
				// show roles in timeline
				try{
					if (frm.doctype=="User" &&  data[key][i][0]=="roles"){
						show_roles(data,version_doc,key,i,out);
						
					}else if (frm.doctype=="User" &&  data[key][i][0]=="block_modules"){
						show_block_modules(data,version_doc,key,i,out)
...
function show_roles(data,version_doc,key,counter,out){
	if(key=="added"){
		d = data[key][counter][1]
		m = `<a href=\"/app/user/${d['modified_by']}\"><b>${d['modified_by']}</b></a><a href=\"/app/version/${version_doc.name}\"> added  the new role <b>${d['role']}</b></a>`
		out.push(get_version_comment(version_doc, m));
	}
	else{
		d = data[key][counter][1]
		m = `<a href=\"/app/user/${d['modified_by']}\"><b>${d['modified_by']}</b></a><a href=\"/app/version/${version_doc.name}\"> removed the role <b>${d['role']}</b></a>`
		out.push(get_version_comment(version_doc, m));
	}
}
function show_block_modules(data,version_doc,key,counter,out){
	if(key=="removed"){
		d = data[key][counter][1]
		m = `<a href=\"/app/user/${d['modified_by']}\"><b>${d['modified_by']}</b></a><a href=\"/app/version/${version_doc.name}\"> added  the new module <b>${d['module']}</b></a>`
		out.push(get_version_comment(version_doc, m));
	}
	else{

		d = data[key][counter][1]
		m = `<a href=\"/app/user/${d['modified_by']}\"><b>${d['modified_by']}</b></a><a href=\"/app/version/${version_doc.name}\"> removed the module <b>${d['module']}</b></a>`
		out.push(get_version_comment(version_doc, m));
	}
}
